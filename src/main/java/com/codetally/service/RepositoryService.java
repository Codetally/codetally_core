...
public class RepositoryService {
    ...
    private void synchAllByOwnername(String ownername) {
        try {
            ...
            if (url != null) {
                ...
                List<Repository> repositoryList = gson.fromJson(new InputStreamReader(urlconnection.getInputStream(), "utf-8"), listType);
    ...
    public Repository getSingleByOwnerAndRepo(String owner, String name) {
        Repository repository = null;
        try {
            repository = repoRepository.getSingleByOwnerAndRepo(owner, name);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return repository;
    ...
    public boolean addSingle(InputStream repositoryInputStream) throws UnsupportedEncodingException {
        Gson gson = new Gson();
        Repository repository = gson.fromJson(new InputStreamReader(repositoryInputStream, "utf-8"), Repository.class);
